    ...
    return (
      <Link to={{ pathname: `/fichelogement/${id}`}} title={title} state={{ from: "occupation " }}> 
        <div id={id} className='flex flex-col items-center rounded-lg bg-kasa text-white p-3 h-full'>
            <img className='border-2 max-w h-auto min-h-200px'  src={pictures[0]} alt={title} />
            <h2 className='flex-auto flex flex-row  align-center items-center text-center'>{title}</h2>
        </div>
      </Link>
    )
}
